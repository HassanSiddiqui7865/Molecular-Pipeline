...
class AntibioticEntry(BaseModel):
    ...
    coverage_for: str = Field(..., description="Specific indication/condition it treats using clinical terminology only (e.g., 'VRE bacteremia', 'MRSA bacteremia', 'uncomplicated cystitis'). Do NOT include ICD codes or ICD code names. Be specific and concise.")
    ...
    dose_duration: Optional[str] = Field(None, description="Dosing information in natural text format. Single: '600 mg IV q12h for 14 days'. Multiple phases: '450 mg q24h on Days 1 and 2, then 300 mg q24h for 7-14 days'. Combinations: 'Trimethoprim 160 mg plus Sulfamethoxazole 800 mg PO q12h for 7 days'. Keep concise - NO monitoring details, NO parenthetical notes, NO infusion rates, NO target levels. Put monitoring/administration details in general_considerations. Use null if not available.")
